...
% cone_combinations/2 - generates combinations from two separate lists
cone_combinations(Top_flavors, Bottom_flavors) ->
    case {Top_flavors, Bottom_flavors} of
        {nil, _} -> fail;  % only fail if first argument is nil
        {[], _} -> [];
        {_, []} -> [];
        {_, nil} -> [];    % return [] if second argument is nil
        _ -> [{T, B} || T <- Top_flavors, B <- Bottom_flavors]
    end.

% cone_combinations/1 - generates all possible combinations from one list
cone_combinations(Flavor_list) ->
    case Flavor_list of
        nil -> fail;
        [] -> [];
        _ -> [{T, B} || T <- Flavor_list, B <- Flavor_list]
    end.

% most_popular_combinations/2 - returns top N popular combinations
most_popular_combinations(Count, List) ->
    case {Count, List} of
        {N, _} when N < 0 -> fail;
        {_, []} -> fail;
        {0, _} -> [];
        {_, L} when length(L) < 5 -> fail;
        {N, L} -> 
            {Result, _} = lists:split(N, L),
            Result
    end.
